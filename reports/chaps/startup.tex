...
When the \texttt{micro:bit} device starts, it reads the initial stack pointer from
...
__extern_C__ void __reset(void)
{
  ...
  /* Set up memory protection for the first half of FLASH */
  ...
  call_constructors();    // Manually call necessary constructors
...
Thus, the first task of \lstinline[style=myagc]|__reset| is to initialize these sections.

The macro \lstinline[style=myagc]|SEC_INIT| copies data from FLASH to RAM, while
\lstinline[style=myagc]|SEC_ZERO| zero-initializes a section:
...
\begin{itemize}
    \item \lstinline[style=myagc]|.recover|: Is retained across soft resets.
    \item \lstinline[style=myagc]|.services|: Stores definitions for service processes
        (e.g., \lstinline[style=myagc]|display|, see \autoref{proc:display}) started at
        every boot.
    \item \lstinline[style=myagc]|.startups|: Contains processes only launched after a
        hardware reset, not during recoveries.
...
\lstinline[style=myagc]|.sys_tests| sections for test objects (\autoref{sec:testing}).
...
However, some constructors (e.g., for \lstinline[style=myagc]|static_list_t|,
\autoref{ds:list}) must run at startup. To support this, we manually define
initializers in a special FLASH section \lstinline[style=myagc]|.init_funcs|, and call
them explicitly via \lstinline[style=myagc,
...
\begin{itemize}
    ...
    \item Startups are launched or recovery routines are executed, depending on the
        boot type.
