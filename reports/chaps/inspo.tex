...
\begin{itemize}[left=0pt]
    \item \textbf{Hardware Selection} The \texttt{micro:bit v1} was chosen as the
        primary target platform. Its Cortex-M0 CPU is among the smallest in the ARM
        family and closely resembles the AGC hardware. 

    \item \textbf{Monolithic Kernel Design} Following Linux's model, Apollo-RTOS adopts a
    ...
    \item \textbf{No Kernel/User Space Isolation} In microcontroller programming, the entire
        program---kernel and applications---is compiled into a single binary. Strict
        isolation would add unnecessary complexity.

    \item \textbf{Library Calls Instead of System Calls} Since Apollo-RTOS does not enforce
    ...
    \item \textbf{Shared Memory IPC} Although shared memory risks race conditions, it offers
        the fastest and simplest form of IPC and is thus adopted as the primary
        mechanism.

    \item \textbf{Memory Usage Philosophy} While the \texttt{micro:bit v1} has limited
        memory, it offers far more than the AGC. Prioritizing maintainability over
        aggressive optimization, we follow a more liberal approach with runtime memory. 
\end{itemize}


We also drew inspiration from the \texttt{microbian} OS for the \texttt{micro:bit v1},
developed by \citet{microbian}, particularly for hardware headers and interrupt
handling primitives.

\subsection{Implementation Architecture}

We now briefly overview Apollo-RTOS's core modules and their interconnections:

\begin{itemize}[left=0em]
    \item \textbf{Scheduler:} The centerpiece of the OS, managing:
        \begin{itemize}[left=0pt]
    ...
    \item \textbf{Waitlist:} Implements a deadline-driven preemptive scheduler (similar to cron
        jobs), executing time-sensitive tasks at precise intervals.

    \item \textbf{Signals, Mutexes, and Barriers:} Enable low-overhead communication
        and synchronization among processes.

    \item \textbf{Memory Allocator:} Handles all dynamic memory management. Allocators maintain
        freelists of memory chunks and serve both runtime heap and stack allocations.

    \item \textbf{File System:} Provides persistent storage through:
        \begin{itemize}[left=0pt]
    ...
    \item \textbf{Error Handling:} Provides diagnostic logging and error detection, including
        runtime assertions and fault recovery.

    \item \textbf{Recovery:} Supports wrapping critical sections and processes with automatic
        recovery against different types of resets.

    \item \textbf{Drivers:} Interface with hardware components:
        \begin{itemize}[left=0pt]
    ...
    \item \textbf{Shell:} Provides a Unix-like command-line interface for user interaction and
        process management.

    \item \textbf{Testing Framework:} Allows registering and running unit and subsystem tests in
        isolation during system startup.

    \item \textbf{``Standard"\footnotemark\ Library:} Offers utility data structures and
        functions, with interfaces inspired by the C++ Standard Library.
\end{itemize}

\footnotetext{In quotation because not much of the standard library is present at the
moment.}
